...
export async function POST(request: NextRequest) {
  try {
    ...
    const pipeline = sharp(buffer).resize(2000, 2000, { fit: 'inside', withoutEnlargement: true })

    let processResult: { data: Buffer; info: sharp.OutputInfo }
    try {
      processResult = await pipeline
        .blur(sigma)
        .raw()
        .toBuffer({ resolveWithObject: true })
    } catch (blurError) {
      console.warn('Blur preprocessing failed, retrying without blur:', blurError)
      processResult = await sharp(buffer)
        .resize(2000, 2000, { fit: 'inside', withoutEnlargement: true })
        .raw()
        .toBuffer({ resolveWithObject: true })
    }

    const { data: processBuffer, info } = processResult
